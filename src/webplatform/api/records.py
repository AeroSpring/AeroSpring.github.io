...
@router.get('/t_{tab_id}', response_model = List[Record])
def get_records(
        tab_id      : int,
        user        : User           = Depends(get_current_user),
        service     : RecordsService = Depends(),
...
@router.get('/r_{record_id}', response_model = Record)
def get_record(
        record_id   : int,
        user        : User           = Depends(get_current_user),
        service     : RecordsService = Depends(),
...
@router.post('/{tab_id}', response_model = Record)
def create_record(
        tab_id      : int,
...
@router.put('/{record_id}/{tab_id}', response_model = Record)
def update_record(
        tab_id      : int,
        record_id   : int,
        record_data : RecordUpdate,
